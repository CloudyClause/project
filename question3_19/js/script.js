...
const STORAGE_KEYS = {
  ...
  selectedRegion:
    "parkingMealV2SelectedRegion"
      ...
      /* 사용자 주차 선호 설정 */
  parkingPreferences:
    "parkingMealV2ParkingPreferences"
...
const appState = {
  ...
  /* ---------------------------------
   ...
   필터와 다르게 데이터를 제거하지 않고
   추천순 점수에 가중치를 주는 데 사용합니다.
--------------------------------- */
parkingPreferences: {

  /* 음식점에서 주차장까지 선호 최대 거리 */
  maxDistance: 500,

  /* 실시간 주차면 정보가 있는 주차장 우선 */
  preferRealtime: true,
...
function cacheDomElements() {
  ...
  DOM.openParkingFilter =
   ...
   Parking Preference
--------------------------------- */
DOM.parkingPreferenceButton =
  document.getElementById(
    "parkingPreferenceButton"
  );

DOM.parkingPreferenceModal =
  document.getElementById(
    "parkingPreferenceModal"
  );

DOM.preferenceRealtimeParking =
  document.getElementById(
    "preferenceRealtimeParking"
  );

DOM.preferenceOpenParking =
  document.getElementById(
    "preferenceOpenParking"
  );

DOM.preferenceFreeParking =
  document.getElementById(
    "preferenceFreeParking"
  );

DOM.parkingPreferenceResetButton =
  document.getElementById(
    "parkingPreferenceResetButton"
  );

DOM.parkingPreferenceSaveButton =
  document.getElementById(
    "parkingPreferenceSaveButton"
  );
  ...
  DOM.parkingDetailAvailability =
    document.getElementById(
      "parkingDetailAvailability"
    );

  DOM.parkingDetailAvailabilityValue =
    document.getElementById(
      "parkingDetailAvailabilityValue"
    );
...
DOM.parkingPreferenceButton
  .addEventListener(
    "click",
    openParkingPreferenceModal
  );

DOM.parkingPreferenceResetButton
  .addEventListener(
    "click",
    resetParkingPreferences
  );
...
try {
  ...
  const savedPreferences =
    JSON.parse(
      localStorage.getItem(
        STORAGE_KEYS.parkingPreferences
      )
    );
  ...
  if (
    ...
    appState.parkingPreferences = {

      maxDistance:
        Number(
          savedPreferences.maxDistance
        ) || 500,

      preferRealtime:
        savedPreferences.preferRealtime
        !== false,

      preferOpen:
        savedPreferences.preferOpen
        !== false,

      preferFree:
        savedPreferences.preferFree
        === true,

      parkingTypes:
        Array.isArray(
          savedPreferences.parkingTypes
        )
          ?
          savedPreferences.parkingTypes
          :
...
function createRestaurantCard(
  ...
  --------------------------------- */
  const parkingStatusArea =
    fragment.querySelector(
      ".restaurant-card__parking-status"
    );


  const parkingStatusValue =
    fragment.querySelector(
      ".restaurant-card__parking-status-value"
    );
  ...
  parkingStatusArea.hidden =
    false;


  parkingStatusValue.textContent =
    availability.label;


  parkingStatusValue.dataset
    .parkingStatus =
    availability.status;
...
const availabilityArea =
  fragment.querySelector(
    ".parking-card__availability-status"
  );


const availabilityValue =
  fragment.querySelector(
    ".parking-card__availability-value"
  );
...
DOM.parkingDetailAvailability.hidden =
  false;
...
/* =========================================================
   Parking Preference Modal
========================================================= */

function openParkingPreferenceModal() {

  syncParkingPreferenceControls();
...
/* =========================================================
   Parking Preference Controls Sync
========================================================= */

function syncParkingPreferenceControls() {
  ...
  document
    .querySelectorAll(
      'input[name="preferenceMaxDistance"]'
    )
    .forEach(
      (input) => {
  ...
  document
    .querySelectorAll(
      'input[name="preferenceParkingType"]'
    )
    .forEach(
      (input) => {
...
function saveParkingPreferences() {
  ...
  const distanceInput =
    document.querySelector(
      'input[name="preferenceMaxDistance"]:checked'
    );
  ...
  const parkingTypes =
    Array.from(
      document.querySelectorAll(
        'input[name="preferenceParkingType"]:checked'
      )
    )
      .map(
        (input) =>
...
/* =========================================================
   Reset Parking Preferences
========================================================= */

function resetParkingPreferences() {
...
/* =========================================================
   ...
   실시간 정보나 총 주차구획 정보가 없으면
   unknown 처리합니다.
========================================================= */

function getParkingAvailabilityStatus(
  parking
) {
